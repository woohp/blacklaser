...
- Tooling uses Vite+ (`vp`) for dev/build/preview/lint/format/check; `vite` is aliased to `@voidzero-dev/vite-plus-core`.
- Dev server: `npm run dev` runs `scripts/vendor-assets.sh` first, then `vp dev` with host `0.0.0.0`.
- Production build: `npm run build` runs `scripts/build-assets.sh`, which vendors WebTorrent's service worker and clears top-level `docs/` before `vp build`.
- Static preview: `npm run preview`.
- Verification: `npm run lint` and `npm run check`; there is no test script in `package.json`.
- Format with `npm run format`; `npm run check` runs `vp check`, `svelte-check`, and `tsc`.
...
- Vite+ lint/format config lives in `vite.config.ts`; there is no separate Biome config.
